 MODULE sms_db
   ...
   REAL(kind=8), ALLOCATABLE :: sms_alp(:),  & !(sms_ns) alpha factor for each set name
                                ...
                                sms_mf(:,:), & !(2,sms_nf) mass and factors
 ...
 ! in MAINPG after damping isms_nformation
    ...
    REAL(kind=8) :: alpha(smax),  & !(sms_ns) alpha factor for each set name
                    ...
                    beta,y(ndime),t(ndime),lg(2),smin,largest
    LOGICAL :: found
    INTEGER(kind=4) :: i,j,k,l,n,ii,jj,kk,ll,nnode,nelem,ip,iel,nar,max_nn,counter
    ...
    ELSE                                  !key-word SELECT exists
      ...
      IF( sms_ns > 0 )THEN                   ! if sets exist
        ...
        IF( sms_nf > 0 )THEN
          ALLOCATE( sms_fibre(sms_nnf,sms_nf), sms_mf(2,sms_nf), sms_faux(ndime,sms_nf))
          sms_fibre = 0 !initializes
          fib => f_head          !point to first
          !for each fibre
          DO j=1,sms_nf
            sms_mf(2,j) = fib%alp  !keep alpha value
 ...
 REAL(kind=8) :: mass,emass(ndofn,npoin)

    IF( sms_nf == 0 )RETURN !no fibres
    sms_mf(1,:) = 0d0    !initializes
    DO i=1,sms_nf        !for each fibre
      DO j=1,sms_nnf        !for each node in the fibre
        ...
        mass = 0d0          !initializes node average mass
        DO l=1,ndime                 !for each space dimension
          ieq = ifpre(l,k)           !global DOF
          !SELECT CASE (ieq)
          !CASE (1: )                 !active DOF
          !  mass = mass + ymass(ieq)   !DOF equivalent mass
          !CASE(-nn:-1 )              !if a slave DOF
          !  mass = mass  + emass(l,k)  !add nodal mass
          !CASE( :-nn )              !if a prescribed DOF
          !  mass = mass  + 1e20        !large mass
          !END SELECT
          IF( ieq < 1 )THEN          !if a prescribed or slave DOF
            mass = mass + emass(l,k)  !add nodal mass
          ELSE                       !if an active DOF
            mass = mass + ymass(ieq)  !DOF equivalent mass
          END IF
        END DO
       sms_mf(1,i) = sms_mf(1,i) + mass !add mass to fiber
      END DO
      sms_mf(1,i) = sms_mf(1,i) / (1d0-sms_mf(2,i)) / ndime   !modify
    END DO
    RETURN
 END SUBROUTINE fibre_mass

 SUBROUTINE dump_sms_data ()
   !called from DUMPIN
   IMPLICIT NONE
   ! local variables
   INTEGER(kind=4) :: i,j
   ...
   WRITE(50,ERR=9999) (sms_mf(:,j),j=1,sms_nf)
